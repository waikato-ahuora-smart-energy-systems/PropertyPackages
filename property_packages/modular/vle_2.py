...
class CubicComplementarityVLE:
    ...
    def phase_equil(b, phase_pair):
        ...
        if len(l_phase) == 0 or len(v_phase) == 0:
    ...
    def calculate_teq(blk, pp):
        ...
        (
            ...
            raoult_comps,
            henry_comps,
            l_only_comps, # added this line
            v_only_comps
        ) = identify_VL_component_list(blk, pp)

        if l_only_comps == []: # fixed this line
        ...
        if v_only_comps == []:
            if blk.is_property_constructed("temperature_dew"):
                Tdew = value(blk.temperature_dew[pp]) # fixed this line
            else:
                Tdew = estimate_Tdew(
                    blk, T_units, raoult_comps, henry_comps, liquid_phase
                )
        ...
        # An initial guess for the equilibrium temperature
