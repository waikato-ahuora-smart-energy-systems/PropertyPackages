...
def thermal_conductivity_rule(m):
    ...
    deltchi = smooth_max(
        delta**2
        * (m.itc("water", delta, tau) - m.itc("water", delta, 1 / Tbr) * Tbr / Tb)
        * m.Pc
        / 1000,
        0,
        eps=1e-8,
    ...
    y = qdb * xi
    kappa = m.cp("water", delta, tau) / m.cv("water", delta, tau)
    Z = (
        2.0
        / math.pi
        / y
    ...
    cpb = m.cp("water", delta, tau) / m.R
    mub = m.mu("water", delta, tau)
...
def main(dry_run=False):
    ...
    Returns:
        WriteParameters
    """
    main_param_file = os.path.join(this_file_dir(), "water.json")
