...
def test_milk_regression():
    # i'm just using pytest because it's easier to run
    pressures = [10000 * 2 ** n for n in range(0, 1)]
    temperatures = [300 + 10 * n for n in range(0, 1)]

    n_items = len(pressures) * len(temperatures)


    m = pyo.ConcreteModel()
    m.fs = FlowsheetBlock(dynamic=False,time_set=range(0,n_items))

    m.fs.reference_enthalpy = pyo.Var(
        units=pyunits.J / pyunits.mol, initialize=0
    )
    m.fs.reference_entropy = pyo.Var(
        units=pyunits.J / pyunits.mol / pyunits.K, initialize=0
    )
    ...
    milk_configuration = {
        ...
        "components": {
            "water": {
                ...
                "parameter_data": {
                    ...
                    "cp_mol_ig_comp_coeff": {
                        ...
                        "H": (0, pyunits.kJ / pyunits.mol), # The H parameter is the heat of formation, which is stupid becuase now I have to change things
                    },
                    ...
                    "entr_mol_form_vap_comp_ref": 
                        (-188.835 * 
                        pyunits.J / pyunits.mol / pyunits.K) + m.fs.reference_entropy,
            ...
            "milk_solid": {
                ...
                "parameter_data": {
                    ...
                    "enth_mol_form_liq_comp_ref": 
                        (-764.8e3 * pyunits.J / pyunits.mol) + m.fs.reference_enthalpy,  # [1]
                    ...
                    "entr_mol_form_liq_comp_ref": 
                        (138.4 * pyunits.J / pyunits.mol / pyunits.K) + m.fs.reference_entropy,  # [1]
                },
            },
        },
        # Specifying phases
        ...
        "pressure_ref": 611.73 * pyunits.Pa, #(101325, pyunits.Pa),
        "temperature_ref": 273.16 * pyunits.K, #(298.15, pyunits.K),
    ...
    m.fs.milk_properties = GenericExtendedParameterBlock(**milk_configuration)
    m.fs.helmholtz_properties = build_package("helmholtz",["water"])
    ...
    m.fs.helmholtz_sb = m.fs.helmholtz_properties.build_state_block(
        m.fs.time,
        defined_state=True,
    )
    # Fix the values for the state variables
    for i in range(0,len(pressures)):
        for j in range(0,len(temperatures)):
            time = i*len(temperatures) + j

            milk_sb = m.fs.milk_sb[time]
            helmholtz_sb = m.fs.helmholtz_sb[time]
            milk_sb.flow_mol.fix(1)
            milk_sb.temperature.fix(temperatures[j])
            milk_sb.pressure.fix(pressures[i])
            milk_sb.mole_frac_comp["milk_solid"].fix(0.01)
            milk_sb.mole_frac_comp["water"].fix(0.99)

            helmholtz_sb.flow_mol.fix(1)
            helmholtz_sb.constrain_component(helmholtz_sb.temperature, temperatures[j])
            helmholtz_sb.pressure.fix(pressures[i])
            

    m.fs.milk_sb.initialize()
    m.fs.helmholtz_sb.initialize()

    m.fs.reference_entropy.fix(0) # only doing reference enthalpy for now

    # define the error function

    m.fs.error = sum(
        (m.fs.milk_sb[time].enth_mol - m.fs.helmholtz_sb[time].enth_mol) ** 2
        for time in [i * len(temperatures) + j]
    )

    print("Initial Error: ", pyo.value(m.fs.error))
    assert degrees_of_freedom(m.fs) == 1
    
    # Optimise to miminise the error
    ...
    print("Final Error: ", pyo.value(m.fs.error))
    print("Reference Enthalpy: ", pyo.value(m.fs.reference_enthalpy))

    m.fs.milk_properties.display()
